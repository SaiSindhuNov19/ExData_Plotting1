...
new[,3]<- as.numeric(as.character(new[,3]))
new[,4]<- as.numeric(as.character(new[,4]))

png("plot4.png",width=480,height=480,units="px",res=NA) 
par(mfrow = c(2,2), mar=(c(4,4,2,1)))
plot(new$Global_active_power~new$dattime, type="l", ylab="Global Active Power (kilowatts)", xlab="")
plot(new$Voltage~new$dattime, type="l", ylab="Voltage", xlab="datetime")
...
plot(new$Global_reactive_power~new$dattime, type="l", ylab="Global_reactive_power", xlab="datetime")
dev.off()
